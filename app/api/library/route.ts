...
export async function GET(req: Request) {
  ...
  return NextResponse.json({ ...out, profile: profile ? { never: profile.never.length, entries: Object.keys(profile.entries).length, updatedAt: profile.updatedAt } : null }, { headers: { "cache-control": device ? "private, max-age=60" : "public, max-age=60" } });
}
